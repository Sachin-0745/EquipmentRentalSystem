...
export default function InventoryManager({ equipmentList, loading, pagination, fetchData, triggerEdit, handleDelete }) {
  return (
    <div>
         ...
         <div className="overflow-x-auto">
           <table className="w-full text-left text-sm">
             <tbody className="divide-y dark:divide-gray-800">
               <TableRowSkeleton columns={6}/><TableRowSkeleton columns={6}/><TableRowSkeleton columns={6}/>
             </tbody>
           </table>
         </div>
                     ...
                     <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                       ...
                       <td className="px-6 py-4">
                         <div className="flex gap-3">
                           <button onClick={() => triggerEdit(item)} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-semibold underline text-xs">Edit</button>
                           <button onClick={() => handleDelete(item.id)} className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 font-semibold underline text-xs">Delete</button>
                         </div>
                       </td>
                     </tr>
                   ))}
